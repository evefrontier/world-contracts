...
//     let mut ts = ts::begin(governor());
//     test_helpers::setup_world(&mut ts);
//     // Creating a storage unit creates a inventory bu default for the owner
//     let storage_unit_id = create_storage_unit(&mut ts);
//     test_helpers::setup_owner_cap_for_user_a(&mut ts, storage_unit_id);
...
//     // Setup inventory owner cap for user_a and user_b
//     ts::next_tx(&mut ts, admin());
//     {
//         let mut storage_unit = ts::take_shared_by_id<StorageUnit>(
//             &ts,
//             storage_unit_id,
//         );
...
//         //Make sure its empty
//         let inv_ref = df::borrow<ID, Inventory>(&ephemeral_storage_unit.id, character_id);
//         assert_eq!(inv_ref.used_capacity(), 0);
//         assert_eq!(inv_ref.remaining_capacity(), MAX_CAPACITY);
//         assert_eq!(inv_ref.inventory_item_length(), 0);
...
//             &mut ephemeral_storage_unit.id,
//             character_id,
//         );
//         eph_inventory.deposit_item(item);

//         let eph_inv_ref = df::borrow<ID, Inventory>(&ephemeral_storage_unit.id, character_id);
//         let used_capacity = (AMMO_QUANTITY as u64) * AMMO_VOLUME;
...
//         let mut ephemeral_storage_unit = StorageUnit {
//             id: uid,
...
//             inventory_keys: vector[],
//         };
//         let inv = inventory::create(ephemeral_storage_unit_id, character_id, 10);
//         ephemeral_storage_unit.inventory_keys.push_back(character_id);
//         df::add(&mut ephemeral_storage_unit.id, character_id, inv);
...
//         let character_id = test_character_id();
//         let inventory = df::borrow_mut<ID, Inventory>(&mut storage_unit_1.id, character_id);
//         inventory.mint_items(
