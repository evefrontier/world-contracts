...
> Vendored from `evefrontier/world-contracts@adr-world-v1:docs/architechture-v1.md`.
> This is the target architecture for the migration. Keep in sync with upstream.
...
Examples: `deposit to SU-01`, `withdraw from SU-01`, `exchange tickets for fuel 1:1`, `jump through gate`.
...
- **Upgrades**: core entity logic and module behavior are decoupled.
- **Reusability**: independent checks (admin approval, sponsor approval, item deposit, fuel burn, proximity proof) can be combined across actions.
- **New module support**: new behavior can be added as a module instead of a new assembly type.
