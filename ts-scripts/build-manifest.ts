...
function sharedVersion(owner: CreatedChange['owner']): string | undefined {
  if (typeof owner === 'object' && owner !== null && 'Shared' in owner) {
    const v = (
      owner as { Shared?: { initial_shared_version?: number | string | null } }
...
function packageFromPublishJson(deployDir: string, pkg: string): PackageEntry {
  ...
  const published = changes.find(
    (c): c is PublishedChange => c.type === 'published',
  )
  if (!published) throw new Error(`no published package in ${pkg}.publish.json`)
  const upgradeCap = changes.find(
    (c): c is CreatedChange =>
      isCreated(c) && c.objectType === UPGRADE_CAP_TYPE,
  )
  if (!upgradeCap)
    throw new Error(`no UpgradeCap created in ${pkg}.publish.json`)
  return {
...
function packageFromPublishedToml(pkg: string, env: string): PackageEntry {
  ...
  const need = (k: string): string => {
    ...
    if (v === undefined || v === null)
      throw new Error(`${path} [published.${env}]: missing ${k}`)
    return String(v)
  }
  return {
...
function addSharedObjects(
  changes: ObjectChange[],
  into: Record<string, SharedObjectEntry>,
): void {
